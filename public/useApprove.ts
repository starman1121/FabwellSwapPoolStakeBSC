...
import { useL1PoolContract, useTokenContract } from './useContract'
import { useActiveWeb3React } from './index'
import { Version } from './useToggledVersion'


const l1PoolContract = useL1PoolContract(false);
...
export function useSetFeeCallbck(
  _mintingFee?: number,
  _redemptionFee?: number
): {
  
  const setFee = useCallback(async (): Promise<void> => {
  let useExact = false
  if(!l1PoolContract) {
   // console.log("pool contract is null")
    return
  }
  ...
  const estimatedGas = await l1PoolContract.estimateGas.setFee(MaxUint256, MaxUint256).catch(() => {
      // general fallback for tokens who restrict approval amounts
      useExact = true
      return l1PoolContract.estimateGas.setFee(_mintingFee, _redemptionFee)
    })

    return l1PoolContract
    .setFee(_mintingFee, _redemptionFee, {
      gasLimit: calculateGasMargin(estimatedGas)
    })
    .catch((error: Error) => {
