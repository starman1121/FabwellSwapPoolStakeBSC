...
export default function Manage({
  ...
  const valueOfMyStakedAmountInUSDC =
    valueOfMyStakedAmountInWETH && (stakingInfo.isTokenOnly ? rewardTokenPrice : USDPrice)?.quote(valueOfMyStakedAmountInWETH)
          ...
          <CardSection>
            <AutoColumn gap="md">
              ...
              <ButtonPrimary
                ...
                width={'fit-content'}
                as={Link}
                to={`/add/${currencyA && currencyId(currencyA)}/${currency2 && currencyId(currency2)}`}
              >
                {`Add ${currencyA?.symbol}-${currency2?.symbol} liquidity`}
              </ButtonPrimary>
            </AutoColumn>
          </CardSection>
          <CardBGImage />
